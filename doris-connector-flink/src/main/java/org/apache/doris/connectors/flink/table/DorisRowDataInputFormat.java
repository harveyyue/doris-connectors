...
public class DorisRowDataInputFormat extends RichInputFormat<RowData, DorisTableInputSplit> implements ResultTypeQueryable<RowData> {
    ...
    public DorisRowDataInputFormat(DorisConfiguration dorisConfig, List<PartitionDefinition> dorisPartitions) {
        this.dorisConfig = dorisConfig;
        this.dorisPartitions = dorisPartitions;
        this.rowDataTypeInfo = TypeInformation.of(new TypeHint<RowData>() {
        });
    }
    ...
    public void open(DorisTableInputSplit inputSplit) throws IOException {
        scalaValueReader = new ScalaValueReader(inputSplit.partition, dorisConfig,
                new FromJavaBiFunction<>((result, schema) -> new FlinkRowBatch(result, schema)));
        rowSize = dorisConfig.getReadFields().split(",").length;
    ...
    public RowData nextRecord(RowData reuse) throws IOException {
        ...
        GenericRowData genericRowData = new GenericRowData(rowSize);
